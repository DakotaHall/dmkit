...
import SettlementOrganizations from "../../create-settlement/SettlementOrganizations/SettlementOrganizations";
import SettlementNPCs from "../../create-settlement/SettlementNPCs/SettlementNPCs";
import SettlementBuildings from "../../create-settlement/SettlementBuildings/SettlementBuildings";
import CreateNPC from "../../create-settlement/CreateNPC";
import SettlementNPC from "../../create-settlement/SettlementNPCs/SettlementNPC";
import SettlementOrganization from "../../create-settlement/SettlementOrganizations/SettlementOrganization";
import SettlementBuilding from "../../create-settlement/SettlementBuildings/SettlementBuilding";
...
class InteractiveSettlement extends Component {
  state = {
    settlementId: window.location.pathname.substr(13, 200),
    name: "",
    image: "",
    population: "",
    environment: "",
  ...
  componentDidUpdate = prevProps => {
    if (prevProps.profile.profile !== this.props.profile.profile) {
      this.setState(
        ...
        () => {
          let neededSettlement = this.state.neededProfile.settlements.find(
            x => x._id === this.state.settlementId
          );
          let newNPCs = this.state.premadeNPCs;
          let editedNPCs = neededSettlement.editedNPCs;
          for (let i = 0; i < editedNPCs.length; i++) {
            let npcIndex = newNPCs.findIndex(x => x._id === editedNPCs[i]._id);
            if (npcIndex === -1) {
              newNPCs.push(editedNPCs[i]);
            } else {
              newNPCs[npcIndex] = editedNPCs[i];
            }
          }
          ...
          let npcList = [];
          for (let i = 0; i < neededSettlement.NPCs.length; i++) {
            let foundNPC = newNPCs.find(
              x => x._id === neededSettlement.NPCs[i]
            );
            if (foundNPC !== undefined) {
              npcList.push(neededSettlement.NPCs[i]);
            }
          }
          let organizations = neededSettlement.organizations;
          let buildings = neededSettlement.buildings;
          for (let i = 0; i < organizations.length; i++) {
            for (
              let l = 0;
              l < organizations[i].organization_leaders.length;
              l++
            ) {
              let foundLeader = newNPCs.find(
                x => x._id === organizations[i].organization_leaders[l]
              );
              if (foundLeader === undefined) {
                organizations[i].organization_leaders.splice(l, 1);
              }
            }
            for (
              let m = 0;
              m < organizations[i].organization_members.length;
              m++
            ) {
              let foundMember = newNPCs.find(
                x => x._id === organizations[i].organization_members[m]
              );
              if (foundMember === undefined) {
                organizations[i].organization_members.splice(m, 1);
              }
            }
          }
          for (let i = 0; i < buildings.length; i++) {
            for (let o = 0; o < buildings[i].building_owners.length; o++) {
              let foundOwner = newNPCs.find(
                x => x._id === buildings[i].building_owners[o]
              );
              if (foundOwner === undefined) {
                buildings[i].building_owners.splice(o, 1);
              }
            }
            for (let w = 0; w < buildings[i].building_workers.length; w++) {
              let foundWorker = newNPCs.find(
                x => x._id === buildings[i].building_workers[w]
              );
              if (foundWorker === undefined) {
                buildings[i].building_workers.splice(w, 1);
              }
            }
          }

          this.setState({
            name: neededSettlement.name,
            image: neededSettlement.image,
            population: neededSettlement.population,
            environment: neededSettlement.environment,
            government: neededSettlement.government,
            commerce: neededSettlement.commerce,
            laws: neededSettlement.laws,
            description: neededSettlement.description,
            organizations: organizations,
            buildings: buildings,
            NPCs: npcList,
            hoveredArray: newHoveredArray,
            greenCheckArray: newGreenCheckArray,
            editedNPCs: neededSettlement.editedNPCs,
            premadeNPCs: newNPCs
          });
        }
      );
  ...
  onSubmit = e => {
    ...
    this.props.editSettlement(
      newSettlement,
      this.state.settlementId,
      this.props.history
    );
  };
        ...
        <div className="ViewPageHolder">
          ...
          >
            <div className="ButtonColorHelper">
              <div className="CenteredPlusNoColor">Save Settlement</div>
            </div>
          </div>
...
InteractiveSettlement.propTypes = {
  editSettlement: PropTypes.func.isRequired,
...
)(withRouter(InteractiveSettlement));
